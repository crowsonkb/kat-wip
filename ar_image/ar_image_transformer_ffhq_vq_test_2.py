...
class Quantizer(nn.Module):
    ...
    def forward(self, x, tau):
        ...
        sample = sample_categorical(logits)
        logp = F.log_softmax(logits, dim=-1).gather(-1, sample[..., None])[..., 0]
        x_rec, log_scale = self.decode(sample)
        return x_rec, log_scale, kl, logp
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        for x, _ in tqdm(dataloader):
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                x_rec, log_scale, loss_kl, logp = quantizer(x, tau)
            loss_kl = loss_kl.flatten(1).sum(dim=1) / x[0].numel()
            ...
            loss_rec = loss_rec.flatten(1).sum(dim=1) / x[0].numel()
            loss = loss_rec + kl_fac * loss_kl
            logp = logp.flatten(1).sum(dim=1)
            box = torch.exp(logp - logp.detach())
            baseline = (loss.sum() - loss) / (loss.numel() - 1)
            loss_reinf = box * loss.detach() + (1 - box) * baseline.detach()
            q_opt.zero_grad()
            torch.mean(loss + loss_reinf).backward()
            q_opt.step()
            dist.all_reduce(loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(loss_rec, op=dist.ReduceOp.AVG)
            dist.all_reduce(loss_kl, op=dist.ReduceOp.AVG)
            print0(
                f"epoch: {epoch}, step: {step}, loss: {loss.mean().item():g}, loss_rec: {loss_rec.mean().item():g}, loss_kl: {loss_kl.mean().item():g}"
            )
            step += 1
        epoch += 1
    ...
    def demo(step):
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_ffhq_vq_010_{epoch:04}_{step:05}.png")
