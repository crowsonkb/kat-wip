...
@compile_wrap
def mixture_of_softmax(x, r):
    return torch.einsum("...rt,...r->...t", x.softmax(-1), r.softmax(-1)).log()


@compile_wrap
def mixture_of_softmax_temp(x, t, r):
    x = x * torch.exp(-t)
    return torch.einsum("...rt,...r->...t", x.softmax(-1), r.softmax(-1)).log()
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.out_temp_proj = nn.Linear(dim, 1, bias=False)
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        for x in xs:
            ...
            temps.append(self.out_temp_proj(x))
            ys.append(self.out_proj(x))
        r = torch.cat(rs, dim=-1)
        y = torch.stack(ys, dim=-2)
        temp = torch.stack(temps, dim=-2)
        return mixture_of_softmax_temp(y, temp, r)
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_ffhq_mos_008_{step:07}.png")
