...
def sample_categorical(logits, tau=1.0):
    ...
    return torch.argmax(logits / tau + gumbel, dim=-1)


def sample_categorical_soft(logits, tau=1.0):
    gumbel = torch.rand_like(logits).log_().nan_to_num_().neg_().log_().neg_()
    return torch.softmax(logits / tau + gumbel, dim=-1)
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.image_embed = nn.Linear(8192, dim, bias=False)
...
def main():
    ...
    # with torch.no_grad():
    #     model_raw.image_embed.weight.copy_(ae.quantize.embed.weight.tile((1, 3)).T)
    ...
    def sample(model, n, y, tau=1.0, cfg_scale=1.0, disable=False):
        n_proc = n // world_size
        x = torch.zeros(n_proc, 0, 8192, device=device)
        ...
        for _ in trange(32 * 32, disable=rank != 0 or disable):
            ...
            sample = sample_categorical_soft(logits, tau=tau)
    ...
    def demo():
        ...
        dist.broadcast(y, 0)
        x = sample(model_ema, 36, y, tau=1.0, cfg_scale=1.0)
        x = rearrange(x, "b (h w) -> b h w", h=32, w=32)
        x = decode(x)
        if rank == 0:
            x = rearrange(x, "(nh nw) c h w -> c (nh h) (nw w)", nh=6, nw=6)
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_imagenet_024_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            x = sample_categorical_soft(x_logits, tau=1.0)
