...
class Transformer(nn.Module):
    ...
    def forward(self, x, y, cache=None, index=None):
        ch = torch.arange(2, device=x.device).tile((x.shape[0], 1024))
        ch_in = ch[:, : x.shape[1]]
        ch_out = ch[:, : x.shape[1] + 1]
        x = self.image_embed(F.one_hot(x, 128).float(), ch_in)
        y = self.class_embed(y)
        x = torch.cat((y[:, None], x), dim=1)
        x = x[:, -1:].contiguous() if cache else x
...
def vqgan_to_twotoken(x):
    return torch.stack((x // 128, x % 128), dim=-1).flatten(-2, -1)


def twotoken_to_vqgan(x):
...
def main():
    ...
    def sample(model, n, y, tau=1.0, disable=False):
        ...
        cache = model.init_cache(n_proc, 32 * 32 * 2, dtype=torch.bfloat16, device=device)
        index = 0
        for _ in trange(32 * 32 * 2, disable=rank != 0 or disable):
    ...
    def demo():
        ...
        x = twotoken_to_vqgan(x)
        x = rearrange(x, "b (h w) -> b h w", h=32, w=32)
        x = decode(x)
        if rank == 0:
            x = rearrange(x, "(nh nw) c h w -> c (nh h) (nw w)", nh=4, nw=4)
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_twotoken_002_{step:07}.png")
