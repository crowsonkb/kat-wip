...
class Quantizer(nn.Module):
    def __init__(self, ch, vocab_size, n_heads):
        ...
        self.act = nn.GELU()
        self.enc_conv_1 = nn.Conv2d(ch, 96, 3, padding=1)
        self.enc_conv_2 = nn.Conv2d(96, 96, 3, padding=1)
        self.enc_conv_3 = nn.Conv2d(96, 96, 3, padding=1)
        self.enc_conv_4 = nn.Conv2d(96, n_heads * vocab_size, 3, padding=1)
        self.dec_in_proj = nn.Conv2d(n_heads * vocab_size, 96, 1, bias=False)
        nn.init.normal_(self.dec_in_proj.weight, std=n_heads ** -0.5)
        self.dec_conv_1 = nn.Conv2d(96, 96, 3, padding=1)
        self.dec_conv_2 = nn.Conv2d(96, 96, 3, padding=1)
        self.dec_conv_3 = nn.Conv2d(96, 96, 3, padding=1)
        self.dec_conv_4 = nn.Conv2d(96, ch * 2, 3, padding=1)
    ...
    def get_kl(self, logits):
        logp = logits.new_tensor(-math.log(logits.shape[-1]))
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        for (x, _) in tqdm(dataloader):
            ...
            tau = math.cos(min(step / n_steps_tau, 1) * math.pi / 2) ** 2
    ...
    def demo(step):
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_cifar_q_s_024_{epoch:04}_{step:05}.png")
