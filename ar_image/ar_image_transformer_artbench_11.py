...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, n_experts):
        super().__init__()
    ...
    def forward(self, x, scores):
        ...
        ids = torch.argmax(scores_g, dim=-1)
        c = torch.softmax(scores, dim=-1).gather(-1, ids[..., None])[..., 0]
        c = c / c.detach()
...
class Block(nn.Module):
    def __init__(self, dim, hidden_dim, head_dim, dropout):
        ...
        self.ff = FeedForward(dim, hidden_dim, 8)

    def forward(self, x, ids, cache=None, index=None):
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, dropout):
        ...
        self.class_scores_embed = nn.Embedding(10 + 1, 8)
        self.image_scores_embed = nn.Embedding(16384, 8)
        ...
        nn.init.zeros_(self.class_scores_embed.weight)
        nn.init.zeros_(self.image_scores_embed.weight)
    ...
    def forward(self, x, y, cache=None, index=None):
        scores = torch.cat((self.class_scores_embed(y[:, None]), self.image_scores_embed(x)), dim=1)
        x = self.image_embed(x)
        y = self.class_embed(y)
        x = torch.cat((y[:, None], x), dim=1)
        x = x[:, -1:].contiguous() if cache and index > 0 else x
        scores = scores[:, -1:].contiguous() if cache and index > 0 else scores
        x = self.embed_drop(x)
        ...
        for block, cache_block in zip(self.blocks, cache):
            ...
            x = block(x, scores, cache_block, index)
        x = self.out_norm(x)
        x = self.out_head(x)
        return x
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_artbench_186_{step:07}.png")
