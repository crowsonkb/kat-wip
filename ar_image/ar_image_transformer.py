...
def sequencematch_loss(
    data_logits, data_tokens, model_logits, model_tokens, alpha, gamma, divergence="chi2-mixture"
):
    ...
    gamma_d = gamma ** torch.arange(1, data_logits.shape[1], device=data_logits.device)
    gamma_m = gamma ** torch.arange(1, model_logits.shape[1], device=model_logits.device)
    l_d = data_logits.gather(2, data_tokens[:, :, None])[:, :, 0]
    v_d = torch.logsumexp(data_logits, dim=2)
    v_m = torch.logsumexp(model_logits, dim=2)

    lv_diff_d = gamma_d * phi(alpha * l_d[:, :-1] - gamma * alpha * v_d[:, 1:]) / alpha
    v_diff_d = -gamma_d * (v_d[:, :-1] - gamma * v_d[:, 1:]) / 2
    v_diff_m = -gamma_m * (v_m[:, :-1] - gamma * v_m[:, 1:]) / 2
    eos_d_1 = gamma_d[-1] / (alpha * (1 - gamma)) * phi(alpha * (1 - gamma) * v_d[:, -1])
    eos_d_2 = -gamma_d[-1] * v_d[:, -1] / 2
    eos_m_2 = -gamma_m[-1] * v_m[:, -1] / 2
    loss_d = lv_diff_d.sum(1) + v_diff_d.sum(1) + eos_d_1 + eos_d_2
    loss_m = v_diff_m.sum(1) + eos_m_2

    if divergence == "chi2-mixture":
        l_m = model_logits.gather(2, model_tokens[:, :, None])[:, :, 0]
        lv_diff_m = gamma_m * (alpha * l_m[:, :-1] - gamma * alpha * v_m[:, 1:]) ** 2
        eos_m_1 = gamma_m[-1] / (1 - gamma) * (alpha * (1 - gamma) * v_m[:, -1]) ** 2
        loss_m = loss_m + lv_diff_m.sum(1) + eos_m_1

    return (loss_d.mean() + loss_m.mean()) * (gamma - 1) / gamma
...
@dataclass
class SequenceMatchOutput:
    state: torch.Tensor
    action: torch.Tensor
    i: torch.Tensor


def sequencematch_loss(s, u, alpha, gamma):
    def phi(x):
        return x - x**2 / (4 * alpha)

    gamma_d = gamma**s.i
    gamma_m = gamma**u.i
    l_d = s.state.gather(-1, s.action[..., None])[..., 0]
    v_d = torch.logsumexp(s.state, dim=-1)
    v_m = torch.logsumexp(u.state, dim=-1)

    lv_diff_d = (
        gamma_d[..., :-1] * phi(alpha * l_d[..., :-1] - gamma * alpha * v_d[..., 1:]) / alpha
    )
    lv_diff_d_eos = (
        gamma_d[..., -1] / (alpha * (1 - gamma)) * phi(alpha * (1 - gamma) * v_d[..., -1])
    )
    ...
    loss_d = lv_diff_d.sum(-1) + lv_diff_d_eos - v_diff_d.sum(-1) - v_diff_d_eos
    loss_m = -v_diff_m.sum(-1) - v_diff_m_eos

    return (loss_d.mean() + loss_m.mean()) * (gamma - 1)
...
def main():
    ...
    def sample(model, n, y, disable=False):
        ...
        cache = model_ema.init_cache(n, 28 * 28, dtype=torch.bfloat16, device=device)
        index = 0
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            ...
            y = y.to(device)
            if step % 100 == 0:
                replay_buffer_x_list.append(sample(model_ema, x.shape[0], y, disable=True))
                replay_buffer_y_list.append(y)
                if len(replay_buffer_x_list) > 100:
                    replay_buffer_x_list.pop(0)
                    replay_buffer_y_list.pop(0)
                model_x = torch.cat(replay_buffer_x_list)
                model_y = torch.cat(replay_buffer_y_list)
            # indices = torch.arange(x.shape[0], device=device)
            indices = torch.randint(model_x.shape[0], (8,), device=device)
            x_in = torch.cat((x, model_x[indices]))
            y_in = torch.cat((y, model_y[indices]))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                # logits = model(x[:, :-1], y).float()
                logits_all = model(x_in[:, :-1], y_in).float()
            logits = logits_all[:batch_size]
            model_logits = logits_all[batch_size:]
            # loss = F.cross_entropy(logits.mT, x)
            # loss = sequencematch_loss(logits, x, logits, x, 0.01, 784 / (784 + 1))
            # loss = sequencematch_loss(logits, x, model_logits, model_x[indices], 0.1, 784 / (784 + 1))
            s = SequenceMatchOutput(logits, x, torch.arange(logits.shape[1], device=device)[None])
            u = SequenceMatchOutput(
                model_logits,
                model_x[indices],
                torch.arange(model_logits.shape[1], device=device)[None],
            )
            loss = sequencematch_loss(s, u, 0.01, 784 / (784 + 1))
            opt.zero_grad()
            loss.backward()
            opt.step()
