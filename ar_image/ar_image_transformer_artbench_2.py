...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, cache=None, index=None):
        ...
        if cache is None:
            ...
            q = apply_rotary_emb(q, self.theta[index:end_index])
            k = apply_rotary_emb(k, self.theta[index:end_index])
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_artbench_002_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            if step % 10 == 0:
                x_m = sample(model_raw, y_drop)
                y_m = y_drop
            x_in = torch.cat((x, x_m))
            y_in = torch.cat((y_drop, y_m))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x_in, y_in).float()
            logits_d = logits[: batch_size]
            logits_m = logits[batch_size :]
            # loss = F.cross_entropy(logits.mT, x)
            q_d = logits_d[:, :-1].gather(-1, x[..., None])[..., 0]
            v_d = torch.logsumexp(logits_d, dim=-1)
            v_m = torch.logsumexp(logits_m, dim=-1)
            term_q_d = -(q_d - v_d[..., 1:])
            term_v_d = (v_d[..., :-1] - v_d[..., 1:]) / 2
            term_v_m = (v_m[..., :-1] - v_m[..., 1:]) / 2
            loss = torch.mean(term_q_d + term_v_d) + torch.mean(term_v_m)
            opt.zero_grad()
            loss.backward()
            opt.step()
