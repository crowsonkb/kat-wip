...
def mixture_of_softmax(x, w):
    ...
    weights = torch.exp(w - torch.logsumexp(w, dim=-2, keepdim=True))
    return torch.log(torch.sum(probs * weights, dim=-2))
...
def main():
    ...
    def sample(model, n, y, tau=1.0, disable=False):
        ...
        index = 0
        for i in trange(32 * 32, disable=rank != 0 or disable):
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_mos_3_007_{step:07}.png")
