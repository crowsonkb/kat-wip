...
def main():
    ...
    @torch.no_grad()
    def sample_for_training(model, y):
        x = y[:, None]
        scale = 1
        scales = 6
        for i in range(scales):
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x)[:, -scale * scale :].float()
    ...
    def sample(model, y, temperature=1.0, top_p=1.0):
        ...
        for i in range(scales):
            ...
            if i > 0:
                x_tmp = torch.cat((x[:, : -scale * scale], sample), dim=1)
            else:
                x_tmp = x
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x_tmp)[:, -scale * scale :].float()
            if top_p < 1.0:
                logits = apply_top_p(logits, top_p)
            sample = sample_categorical(logits, temperature)
            if i < scales - 1:
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            x = x.to(device)
            y = y.to(device)
            inps_1, tgts = prepare_seqs(x, y, lambda x: quantize(x.movedim(1, 3), palette))
            inps_2 = sample_for_training(model, y)
            inps = torch.cat((inps_1, inps_2), dim=0)
            tgts = tgts.tile((2, 1))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(inps)
