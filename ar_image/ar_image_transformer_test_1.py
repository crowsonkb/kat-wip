...
def mixture_of_tempered_softmax(x, t, r):
    x = x * torch.exp(-t)
    return torch.einsum("...rt,...r->...t", x.softmax(-1), r.softmax(-1)).log()
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        # self.out_up = nn.Linear(dim, hidden_dim)
        # self.out_proj = nn.Linear(hidden_dim, 256)
        self.out_router = nn.Linear(dim, 1)
        self.out_temp = zero_init(nn.Linear(dim, 1))
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        # x = self.out_up(x)
        # x = F.silu(x)
        # x, y = self.out_up(x).chunk(2, dim=-1)
        # x = x * F.silu(y)
        # return self.out_proj(x)
        # r = self.out_router(x).unsqueeze(-1)
        # x = self.out_proj(x).view(*x.shape[:2], 8, 256)
        # sr = F.softmax(r, dim=-2)
        # sx = F.softmax(x, dim=-1)
        # x = torch.sum(sr * sx, dim=-2)
        # return torch.log(x)
        xs = xs[-2:]
        ys, rs, ts = [], [], []
        for x in xs:
            x = self.out_norm(x)
            rs.append(self.out_router(x))
            ts.append(self.out_temp(x))
            ys.append(self.out_proj(x))
        r = torch.cat(rs, dim=-1)
        y = torch.stack(ys, dim=-2)
        t = torch.stack(ts, dim=-2)
        return mixture_of_tempered_softmax(y, t, r)
