...
@compile_wrap
def mixture_of_tempered_softmax(x, t, w):
    x = x * t[..., None]
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, dropout):
        ...
        self.out_router = nn.Linear(dim, self.n_softmax, bias=False)
        self.out_temp = nn.Linear(dim, self.n_softmax, bias=False)
        self.out_proj = DMoELinear(dim, 8192, self.n_softmax, bias=False)
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        x = self.out_norm(x)
        w = self.out_router(x)
        t = self.out_temp(x) / 10 + 1
        w_topk, w_index = torch.topk(w, self.k_softmax, dim=-1)
        t_topk = t.gather(-1, w_index)
        x_topk = self.out_proj(x[..., None, :].expand(-1, -1, self.k_softmax, -1), w_index)
        x = mixture_of_tempered_softmax(x_topk, t_topk, w_topk)
        return x
...
def main():
    ...
    dim = 1024
    hidden_dim = round_to_power_of_two(dim * 8 / 3, 0.05)
    model_raw = Transformer(24, dim, hidden_dim, 64, dropout=0.0).to(device)
    ...
    dataset = datasets.ImageFolder("/scratch/kat/ilsvrc2012/train", transform=transform)
    ...
    def sample(model, n, y, tau=1.0, cfg_scale=1.0, disable=False):
        ...
        y = torch.cat((y, y.new_zeros(n_proc * world_size - y.shape[0])))
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.float().cpu()).save(f"demo_imagenet_020_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            x_tokens = sample_categorical(x_logits)
            y_drop = torch.where(torch.rand_like(y, dtype=torch.float32) < 0.1, 1000, y)
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x_tokens[:, :-1], y_drop).float()
