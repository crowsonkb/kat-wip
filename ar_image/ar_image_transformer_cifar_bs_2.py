...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, pos, mask=None, cache=None, index=None):
        x = self.norm(x)
        ...
        if index is None:
            ...
            x = F.scaled_dot_product_attention(q, k, v, mask[:, None], 0.0 if self.training else 0)
...
class Block(nn.Module):
    ...
    def forward(self, x, pos, mask=None, cache=None, index=None):
        x = x + self.attn(x, pos, mask, cache, index)
        x = x + self.ff(x)
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos, mask=None, cache=None, index=None):
        ...
        for block, cache_block in zip(self.blocks, cache):
            ...
            x = block(x, pos, mask, cache_block, index)
        x = self.out_norm(x)
        x = self.out_proj(x)
        return x
...
def main():
    ...
    def demo(step):
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_cifar_bs_2_022_{epoch:04}_{step:05}.png")
    ...
    while True:
        ...
        for step, ((x, tgt, mask, attn_mask, pos), y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            if step % 100 == 0:
                demo(step)
            ...
            attn_mask_ = torch.ones(x.shape[0], mask.shape[1] + 1, mask.shape[1] + 1, dtype=torch.bool, device=device).tril()
            attn_mask_[:, 1:, 1:] = attn_mask
            attn_mask = attn_mask_[:, :-1, :-1]
            pos = torch.cat((torch.zeros_like(y[:, None]), pos), dim=1)[:, :-1]
            x = torch.cat((y[:, None], x), dim=1)[:, :-1]

            # model_raw.eval()
            with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits_for_noise = model_ema(x, pos, attn_mask).float()
            # model_raw.train()
            ...
            samples = torch.argmax(logits_for_noise + gumbel * 100, dim=-1)
            x[:, 1:] = torch.where(mask[:, :-1], x[:, 1:], samples[:, :-1])
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x, pos, attn_mask).float()
