...
class SelfAttention(nn.Module):
    def __init__(self, dim, head_dim):
        ...
        # pos = make_pos_emb(28, 29, self.n_heads * head_dim // 2)
        # pos = torch.cat((torch.zeros(1, self.n_heads * head_dim // 2), pos))
        # self.register_buffer("theta", pos.view(-1, head_dim // 2, self.n_heads).mT)
...
def make_pos_emb(h, w, dim):
    ...
    d_isqrt = torch.zeros(h * w, h * w)
    for i, node in enumerate(g):
        for edge in node:
            a[i, edge] = 1
            d_isqrt[i, i] = 1 / math.sqrt(len(node))
    eye = torch.eye(h * w)
    lap = eye - d_isqrt @ a @ d_isqrt
    vals, vecs = torch.linalg.eigh(lap)
    # proj = torch.nn.init.orthogonal_(torch.empty(dim, h * w), math.sqrt(h * w / dim))
    return vecs[:dim, :].T * math.sqrt(h * w)
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        pos = make_pos_emb(28, 29, dim)
        pos = torch.cat((torch.zeros(1, dim), pos))
    ...
    def forward(self, x, m, p, cache=None, index=None):
        ...
        x = torch.cat((y, x), dim=1) + self.pos_emb[p]
        b_idx = torch.arange(x.shape[0], device=x.device)
...
def main():
    ...
    model_raw = Transformer(4, 128, 256, 32).to(device)
