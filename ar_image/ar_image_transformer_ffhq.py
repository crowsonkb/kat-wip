...
class MonarchMatrix(nn.Module):
    def __init__(self, sqrt_n: int):
        super().__init__()
        self.sqrt_n = sqrt_n
        self.L = nn.Parameter(torch.randn((sqrt_n, sqrt_n, sqrt_n)) * sqrt_n ** -0.5)
        self.R = nn.Parameter(torch.randn((sqrt_n, sqrt_n, sqrt_n)) * sqrt_n ** -0.5)

    def extra_repr(self):
        return f"sqrt_n={self.sqrt_n}"
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, dropout):
        ...
        # self.out_temp = nn.Linear(dim, 1, bias=False)
        self.out_up = nn.Linear(dim, 16384, bias=False)
        nn.init.orthogonal_(self.out_up.weight, gain=math.sqrt(16384 / dim))
        self.out_act = nn.Tanh()
        self.out_proj = MonarchMatrix(128)
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        x = self.out_norm(x)
        # temp_output = self.out_temp(x)
        x = self.out_up(x)
        x = self.out_act(x)
        x = self.out_proj(x)
        # x = x * (temp_output / 10 + 1)
        return x
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_035_{step:07}.png")
