...
def make_axial_pos_final(h, w, d, dtype=None, device=None):
    ...
    for i in range(d):
        ...
        pos_rest = torch.full((pos_ax.shape[0], 1), (i + 1) / d, dtype=dtype, device=device)
...
class SelfAttention(nn.Module):
    def __init__(self, dim, head_dim):
        ...
        pos = make_axial_pos_final(32, 32, 4)
...
def main():
    ...
    def sample(model, n, y, tau=1.0, cfg_scale=1.0, disable=False):
        ...
        n_tokens = 32 * 32 + 16 * 16 + 8 * 8 + 4 * 4
        cache = model_ema.init_cache(n_proc * 2, n_tokens, dtype=torch.bfloat16, device=device)
        index = 0
        for _ in trange(n_tokens, disable=rank != 0 or disable):
            x_in = torch.cat((x, x))
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_imagenet_multiscale_001_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            x_1 = x
            x_1_logits = encode(x_1).flatten(1, 2).float()
            x_2 = F.interpolate(x_1, scale_factor=0.5, mode="bilinear", align_corners=False, antialias=True)
            x_2_logits = encode(x_2).flatten(1, 2).float()
            x_3 = F.interpolate(x_2, scale_factor=0.5, mode="bilinear", align_corners=False, antialias=True)
            x_3_logits = encode(x_3).flatten(1, 2).float()
            x_4 = F.interpolate(x_3, scale_factor=0.5, mode="bilinear", align_corners=False, antialias=True)
            x_4_logits = encode(x_4).flatten(1, 2).float()
            x_logits = torch.cat((x_4_logits, x_3_logits, x_2_logits, x_1_logits), dim=1)
