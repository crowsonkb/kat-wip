...
def thing3a(logits, targets, phi):
    q = logits.gather(-1, targets[..., None])[..., 0]
    v = torch.logsumexp(logits, dim=-1)
    qv_diff = -phi(q[..., :-1] - v[..., 1:])
    v_diff = v[..., :-1] - v[..., 1:]
    return qv_diff + v_diff


def thing3b(logits, targets, γ, φ):
    γ_i = γ**torch.arange(1, targets.shape[-1] + 1, device=logits.device)
    q = logits.gather(-1, targets[..., None])[..., 0]
    v = torch.logsumexp(logits, dim=-1)
    qv_diff = γ_i[..., :-1] * -φ(q[..., :-1] - γ * v[..., 1:])
    qv_eos = γ_i[..., -1] * -φ((1 - γ) * v[..., -1]) / (1 - γ)
    v_diff = γ_i[..., :-1] * (v[..., :-1] - γ * v[..., 1:])
    v_eos = γ_i[..., -1] * v[..., -1]
    return qv_diff.sum(-1) + qv_eos + v_diff.sum(-1) + v_eos


def thing3c(logits, targets, phi):
    q = logits.gather(-1, targets[..., None])[..., 0]
    v = torch.logsumexp(logits, dim=-1)
    qv_diff = -phi(q[..., :-1] - v[..., 1:])
    v_diff = (v[..., :-1] - v[..., 1:]) / 2
...
def main():
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            ...
            y = y.to(device)
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x, y).float()
            xent = F.cross_entropy(logits[:, :-1].detach().mT, x)
            tgt = torch.cat((x, torch.zeros_like(x[:, -1:])), dim=1)
            loss = thing3b(logits, tgt, 784 / (784 + 1), lambda x: x - x**2 / 4).mean() / 784
            opt.zero_grad()
            loss.backward()
            opt.step()
