...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.out_head = nn.Linear(dim, 256)
...
def prepare_seqs(x, y):
    tgts, inps, tgts_real, inps_real = [], [], [], [y[:, None]]
    while True:
        tgts.insert(0, x)
        ...
        x = F.avg_pool2d(x, 2)
    for tgt in tgts[:-1]:
        inps.append(F.interpolate(tgt, scale_factor=2, mode="nearest"))
    for tgt in tgts:
        tgts_real.append(tgt.movedim(1, 3).flatten(1).mul(255).clamp(0, 255).round().long())
    for inp in inps:
        inps_real.append(inp.movedim(1, 3).flatten(1).mul(255).clamp(0, 255).round().long())
    return torch.cat(inps_real, dim=-1), torch.cat(tgts_real, dim=-1)
...
def main():
    ...
    model_raw = Transformer(8, 384, 1024, 64).to(device)
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            inps, tgts = prepare_seqs(x.to(device), y.to(device))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(inps)
                loss = F.cross_entropy(logits.mT, tgts)
            opt.zero_grad()
            loss.backward()
            opt.step()
            sched.step()
