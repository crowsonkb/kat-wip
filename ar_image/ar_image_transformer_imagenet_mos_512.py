...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.heads = nn.ModuleList([Head(dim, 8192) for _ in range(3)])
...
def main():
    ...
    def sample(model, n, y, tau=1.0, cfg_scale=1.0, disable=False):
        ...
        cache = model.init_cache(n_proc * 2, 64 * 64, dtype=torch.bfloat16, device=device)
        index = 0
        for _ in trange(64 * 64, disable=rank != 0 or disable):
            x_in = torch.cat((x, x))
    ...
    def demo():
        y = torch.randint(1000, (16,), device=device)
        dist.broadcast(y, 0)
        x = sample(model_ema, 16, y, tau=1.0, cfg_scale=2.0)
        x = rearrange(x, "b (h w) -> b h w", h=64, w=64)
        x = decode(x)
        if rank == 0:
            x = rearrange(x, "(nh nw) c h w -> c (nh h) (nw w)", nh=4, nw=4)
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_imagenet_mos_512_001_{step:07}.png")
