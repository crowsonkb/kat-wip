...
class SelfAttention(nn.Module):
    def __init__(self, dim, head_dim):
        ...
        pos = make_axial_pos_final(32, 32, 3)
...
class Head(nn.Module):
    def __init__(self, dim, vocab_size):
        ...
        self.act = nn.GELU()
        self.proj = nn.Linear(dim * 4, vocab_size, bias=False)
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        # self.heads = nn.ModuleList([Head(dim, 16384) for _ in range(3)])
...
def main():
    ...
    # base_path = "/home/kat/text-to-image/latent-diffusion/models/first_stage_models/vq-f16"
    base_path = "/weka/kat/ar_image/latent-diffusion/models/first_stage_models/vq-f16"
    config_path = f"{base_path}/config.yaml"
    model_path = f"{base_path}/model.ckpt"
    ...
    dataset = FolderOfImages("/scratch/kat/images1024x1024", transform=transform)
    ...
    def sample(model, n, y, tau=1.0, disable=False):
        ...
        cache = model_ema.init_cache(n_proc, n_tokens, dtype=torch.bfloat16, device=device)
        index = 0
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_multiscale_006_{step:07}.png")

    while True:
        sampler.set_epoch(epoch)
        for x_1, in tqdm(dataloader, disable=rank > 0):
            if step > 0 and step % 500 == 0:
                demo()
            x_1 = x_1.to(device)
            x_1e = encode(x_1)
            x_2 = F.avg_pool2d(x_1, 2)
            x_2e = encode(x_2)
            x_3 = F.avg_pool2d(x_2, 2)
            x_3e = encode(x_3)
            x = torch.cat((x_3e, x_2e, x_1e), dim=1)
