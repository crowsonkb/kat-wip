...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        x = F.scaled_dot_product_attention(
            q, k, v, self.mask[: x.shape[1], : x.shape[1]], 0.1 if self.training else 0
        )
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, vocab_size, sizes):
        ...
        self.pos_embed = nn.Embedding(sum(size**2 for size in sizes), dim)
...
def prepare_seqs(x, y, sizes, encode_fn):
    tgts = []
    for size in sizes:
        x_s = F.interpolate(x, (size, size), mode="bilinear", align_corners=False, antialias=True)
        # x_s = F.adaptive_avg_pool2d(x, (size, size))
        tgts.append(encode_fn(x_s).flatten(1))
    tgt = torch.cat(tgts, dim=-1)
    inp = y[:, None].expand_as(tgt)
    # inps.append(y[:, None])
    # for tgt, old_size, new_size in zip(tgts[:-1], sizes[:-1], sizes[1:]):
    #     inp = rearrange(tgt, "n (h w) -> n 1 h w", h=old_size, w=old_size)
    #     inp = F.interpolate(inp.float(), (new_size, new_size), mode="nearest")
...
def main():
    ...
    model_raw = Transformer(8, 512, 1360, 64, vocab_size, sizes).to(device)
    ...
    def sample(model, y, temperature=1.0, top_p=1.0):
        ...
        for i, size in enumerate(sizes):
            ...
            if i < len(sizes) - 1:
                next_size = sizes[i + 1]
                last_scale = rearrange(sample.float(), "n (h w) -> n 1 h w", h=size, w=size)
                last_scale = F.interpolate(last_scale, (next_size, next_size), mode="nearest")
                x = torch.cat((x, last_scale.flatten(1).long()), dim=1)
        return sample

    def demo():
        y = torch.arange(10, device=device).repeat_interleave(10)
        x = sample(model_ema, y, top_p=0.95)
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            x = x.to(device)
            y = y.to(device)
            inps, tgts = prepare_seqs(x, y, sizes, lambda x: quantize(x.movedim(1, 3), palette))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(inps)
