...
@torch.compile
def sm_loss(logits, targets, alpha, gamma):
    def phi(x):
    ...
    return (qv_diff.sum(-1) + qv_eos + v_diff.sum(-1) + v_eos) / targets.shape[-1]
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_artbench_068_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            loss = sm_loss(logits, x, alpha=1.0, gamma=1024 / (1024 + 1)).mean()
            opt.zero_grad()
            loss.backward()
            opt.step()
