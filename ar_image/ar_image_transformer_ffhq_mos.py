...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.out_router = nn.Linear(dim, 8, bias=False)
        self.out_proj = nn.Linear(dim, 16384 * 8, bias=False)
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        x = self.out_norm(x)
        r = self.out_router(x)
        x = self.out_proj(x).view(*x.shape[:-1], 8, -1)
        return mixture_of_softmax(x, r)
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_ffhq_mos_004_{step:07}.png")
