...
class DMoELinear(nn.Module):
    def __init__(self, in_features, out_features, n_experts, bias=True):
        ...
        self.experts = nn.ModuleList([nn.Linear(in_features, out_features, bias=bias) for _ in range(n_experts)])

    def forward(self, x, ids):
        out_shape = (*x.shape[:-1], self.out_features)
        x = x.flatten(0, -2)
        ids = ids.flatten()
        _, ids_indices = torch.sort(ids)
        outs = []
        for i in range(self.n_experts):
            x_in = x[ids == i]
            out = self.experts[i](x_in)
            outs.append(out)
        out = torch.cat(outs)
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, dropout):
        ...
        self.out_router = zero_init(nn.Linear(dim, self.n_softmax, bias=False))
        self.out_proj = DMoELinear(dim, 16384, self.n_softmax, bias=False)
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        x = self.out_proj(x[..., None, :].expand(-1, -1, self.k_softmax, -1), w_index)
        x = mixture_of_softmax(x, w_topk)
        return x
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_mos_5_002_{step:07}.png")
