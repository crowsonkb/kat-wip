...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, vocab_size, dropout):
        ...
        self.out_proj = zero_init(nn.Linear(dim, vocab_size, bias=False))
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_cifar_q_corrupt_003_{epoch:04}.png")
    ...
    while True:
        ...
        for step, (x, y) in enumerate(tqdm(dataloader, disable=rank > 0)):
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16), torch.no_grad():
                logits_for_corruption = model(x[:, :-1], y).float()
            x_for_corruption = sample_categorical(logits_for_corruption)
            x_keep = torch.rand_like(x_for_corruption, dtype=torch.float32) > 0.1
            x_corrupt = torch.where(x_keep, x, x_for_corruption)
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x_corrupt[:, :-1], y).float()
