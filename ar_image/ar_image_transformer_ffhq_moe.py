...
class MoELinear(nn.Module):
    def __init__(self, in_features, out_features, n_experts, k):
        super().__init__()
        self.k = k
    ...
    def forward(self, x, logits):
        x = torch.einsum("nsi,eoi->nseo", x, self.weight)
        values, indices = logits.softmax(dim=-1).topk(self.k, dim=-1)
        indices = indices[..., None].expand(x.shape[0], x.shape[1], indices.shape[2], x.shape[3])
        return torch.einsum("nseo,nsf->nso", x.gather(-2, indices), values)
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, n_experts, k):
        ...
        self.up = MoELinear(dim, hidden_dim * 2, n_experts, k)
        self.act = nn.SiLU()
        self.down = zero_init(MoELinear(hidden_dim, dim, n_experts, k))
    ...
    def forward(self, x):
        ...
        x = self.up(x, logits)
        x = swiglu(x)
        x = self.down(x, logits)
        return x + skip


class Block(nn.Module):
    def __init__(self, dim, hidden_dim, head_dim, n_experts, k):
        super().__init__()
        self.attn = SelfAttention(dim, head_dim)
        self.ff = FeedForward(dim, hidden_dim, n_experts, k)
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, n_experts, k):
        super().__init__()
        self.depth = depth
        self.dim = dim
        ...
        self.blocks = nn.ModuleList([Block(dim, hidden_dim, head_dim, n_experts, k) for _ in range(depth)])
        self.out_norm = RMSNorm((dim,))
        self.out_proj = nn.Linear(dim, 16384, bias=False)
        # self.out_projs = nn.ModuleList([OutProj(dim, 16384) for _ in range(n_out_projs)])
    ...
    def forward(self, x, y, cache=None, index=None):
        ...
        x = self.out_norm(x)
        x = self.out_proj(x)
        # xs = xs[-self.n_out_projs:]
        # x = torch.stack([proj(x) for x, proj in zip(xs, self.out_projs)], dim=-2).logsumexp(dim=-2)
        return x
...
def main():
    ...
    model_raw = Transformer(12, 768, 2048, 64, n_experts=8, k=2).to(device)
    with torch.no_grad():
        ...
        # for out_proj in model_raw.out_projs:
        #     proj = nn.init.orthogonal_(torch.empty(8, 768, device=device), math.sqrt(8 / 16384))
        #     out_proj.proj.weight.copy_(ae.quantize.embedding.weight @ proj)
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_ffhq_moe_001_{step:07}.png")
