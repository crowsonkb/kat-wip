...
class HashFeedForward(nn.Module):
...
class Block(nn.Module):
    def __init__(self, dim, hidden_dim, head_dim, dropout):
        ...
        self.ff = HashFeedForward(dim, hidden_dim, 8)
...
class Head(nn.Module):
    def __init__(self, dim, n_classes, n_experts):
        super().__init__()
        self.dim = dim
        self.n_classes = n_classes
        self.n_experts = n_experts
        self.norm = RMSNorm((dim,))
        self.proj = GroupedLinear(dim, n_classes, n_experts, bias=False)

    def embed(self, x, ids):
        weight = rearrange(self.proj.weight, "n o i -> o (n i)")
        embeds = rearrange(F.embedding(x, weight), "... (n i) -> ... n i", n=self.n_experts)
        return torch.mean(embeds, dim=-2)
        # return embeds.gather(-2, ids[..., None, None].expand(-1, -1, -1, self.dim))[..., 0, :]
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim, dropout):
        ...
        self.out_head = Head(dim, 16384, 8)
    ...
    def forward(self, x, y, cache=None, index=None):
        ids = torch.cat((y[:, None] % 8, x % 8), dim=1)
        # x = self.image_embed(x)
        x = self.out_head.embed(x, ids[:, 1:])
        y = self.class_embed(y)
        x = torch.cat((y[:, None], x), dim=1)
        x = x[:, -1:].contiguous() if cache and index > 0 else x
        ids = ids[:, -1:].contiguous() if cache and index > 0 else ids
        x = self.embed_drop(x)
        ...
        for block, cache_block in zip(self.blocks, cache):
            ...
            x = block(x, ids, cache_block, index)
        # x = self.out_norm(x)
        x = self.out_head(x, ids)
        return x
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu().float()).save(f"demo_artbench_187_{step:07}.png")
