...
def simple_sm_loss(logits, tokens, gamma=None):
    def phi(x):
        return x - x**2 / 4

    if gamma is None:
        gamma = tokens.shape[1] / (tokens.shape[1] + 1)

    gamma_i = gamma ** torch.arange(1, tokens.shape[1], device=tokens.device)
    l = logits.gather(2, tokens[:, :, None])[:, :, 0]
    v = torch.logsumexp(logits, dim=2)
    lv_diff = gamma_i * phi(l[:, :-1] - gamma * v[:, 1:])
    v_diff = -gamma_i * (v[:, :-1] - gamma * v[:, 1:])
    eos = gamma_i[-1] * phi((1 - gamma) * v[:, -1]) / (1 - gamma) - gamma_i[-1] * v[:, -1]
    loss = lv_diff.sum(1) + v_diff.sum(1) + eos
    return -loss.mean()


def simple_sm_loss_soft(logits, targets, gamma=None):
    def phi(x):
        return x - x**2 / 4

    if gamma is None:
        gamma = targets.shape[1] / (targets.shape[1] + 1)

    gamma_i = gamma ** torch.arange(1, targets.shape[1], device=targets.device)
    v = torch.logsumexp(logits, dim=2)
    lv_diff = gamma_i * torch.sum(targets[:, :-1] * phi(logits[:, :-1] - gamma * v[:, 1:, None]), dim=2)
    v_diff = -gamma_i * (v[:, :-1] - gamma * v[:, 1:])
    eos = gamma_i[-1] * phi((1 - gamma) * v[:, -1]) / (1 - gamma) - gamma_i[-1] * v[:, -1]
    loss = lv_diff.sum(1) + v_diff.sum(1) + eos
    return -loss.mean()
...
def main():
    ...
    proj = nn.init.orthogonal_(torch.empty(256, 768, device=device))
    with torch.no_grad():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_imagenet_021_{step:07}.png")
    ...
    while True:
        ...
        for x, y in tqdm(dataloader, disable=rank > 0):
            ...
            # if step % 100 == 0:
            #     model_x = sample_for_training(model_ema, x.shape[0] * 2, torch.tile(y, (2,)))
            # indices = torch.randint(model_x.shape[0], (8,), device=device)
            # x_in = torch.cat((x, model_x[indices]))
            # y_in = torch.cat((y, y[indices]))
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                logits = model(x[:, :-1], y).float()
                # logits_all = model(x_in[:, :-1], y_in).float()
            # logits = logits_all[:x.shape[0]]
            # model_logits = logits_all[x.shape[0]:]
            # loss = sequencematch_loss(logits, x, model_logits, model_x[indices], 0.01, 1024 / (1024 + 1))
            # loss = simple_sm_loss(logits, x)
            loss = simple_sm_loss_soft(logits, x_logits.softmax(dim=-1))
            opt.zero_grad()
            loss.backward()
            opt.step()
