...
class PLIF(nn.Module):
    def __init__(self, t, k):
        ...
        self.register_buffer("l", torch.linspace(-t, t, k + 1))

    def extra_repr(self) -> str:
        return f"t={self.t}, k={self.k}"

    def forward(self, x):
        i = torch.floor((x + self.t) * self.k / self.t / 2).clamp(0, self.k - 1).long()
        s = F.softplus(self.weight)
        s_cumsum = F.pad(torch.cumsum(s, 0), (1, 0))
        s_i = s.gather(0, i.flatten()).reshape(x.shape)
        s_cumsum_i = s_cumsum.gather(0, i.flatten()).reshape(x.shape)
        l_i = self.l.gather(0, i.flatten()).reshape(x.shape)
        b_i = self.bias + s[0] + self.l[0] - l_i * s_i + s_cumsum_i * 2 * self.t / self.k
        return s_i * x + b_i
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_imagenet_plif_001_{step:07}.png")
