...
def sequencematch_loss(
    ...
    lv_diff_d_eos = gamma_d[-1] / (alpha * (1 - gamma)) * phi(alpha * (1 - gamma) * v_d[:, -1])
    ...
    loss_d = lv_diff_d.sum(1) + v_diff_d.sum(1) + lv_diff_d_eos + v_diff_d_eos
    ...
    if divergence == "chi2-mixture":
        ...
        lv_diff_m_eos = -gamma_m[-1] / (1 - gamma) * (alpha * (1 - gamma) * v_m[:, -1]) ** 2
        loss_m = loss_m + lv_diff_m.sum(1) + lv_diff_m_eos
...
class Transformer(nn.Module):
    def __init__(self, depth, dim, hidden_dim, head_dim):
        ...
        self.out_proj = zero_init(nn.Linear(dim, 16384, bias=False))
...
def main():
    ...
    def demo():
        ...
        if rank == 0:
            ...
            x = torch.clamp(x, 0, 1)
            TF.to_pil_image(x.cpu()).save(f"demo_ffhq_009_{step:07}.png")
    ...
    while True:
        ...
        for x, in tqdm(dataloader, disable=rank > 0):
            ...
            i = torch.arange(logits.shape[1], device=device)[None]
            s = SequenceMatchInput(logits, x, i)
            u = SequenceMatchInput(model_logits, model_x[indices], i)
            loss = sequencematch_loss(s, u, 0.01, 1024 / (1024 + 1))
            opt.zero_grad()
            loss.backward()
            opt.step()
